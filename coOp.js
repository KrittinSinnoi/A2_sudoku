...
function keyPressed(){
  if(selected.c >= 0 && selected.r >= 0){
    if( key >= '1' && key <= '9' ){
      ...
      //ownerBoard[selected.r][selected.c] = player;
      checkSolution();
      
      let before = wrongCells.size;
      if(wrongCells.size > before){
        score[player] -= 1; // worng
      }else{
        score[player] += 1; // corect
      }
      ...
      ownerBoard[selected.r][selected.c] = 0;
    }
  }
}
